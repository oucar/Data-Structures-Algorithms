...
public:
	...
	int set(int index, int x);							// Set an index to a given element
	...
	void insert(int index, int x);						// Insert an element to a given index

	int linearSearch(int key);							// Linear search
	int binarySearch(int key);							// Binary search
	int recursiveBinarySearch(int key);					// **Recursive** binary search

	int max();											// Maximum element
	int min();											// Minimum element
	int sum();											// Sum of all the elements
	int average();										// Average of all the elements
	...
	Array* differenceOperation(Array* arrGiven);		// Finds the difference of two array and returns it as an array
														...
														//TODO 1 Create as many instances as you want and make them accesible through the menu.
};
